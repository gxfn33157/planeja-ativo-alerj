...
async function startSimulado() {
    ...
    if (!questions || questions.length === 0) return alert('Sem questões carregadas.');
    
    // Shuffle and pick 80 unique
...
function renderQuestion() {
    ...
    q.alts.forEach((alt, idx) => {
        ...
        div.innerText = alt.l || alt; // Handle object or string
...
function quitSimulado() {
    if (confirm('Sair do simulado? Progresso será perdido.')) loadDashboard();
}
...
async function finishSimulado() {
    ...
    // Correct
    let score = 0;
    let subjectStats = {}; // { materia: { total: 0, correct: 0, contents: {} } }
